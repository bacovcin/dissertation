...
\trans `He sent Maria a letter, but it has not reached her.'\pagebreak}
...
German also provides evidence that the dative recipients and prepositional goals are syntactically distinct. While German has fairly free word order, topicalised VPs provide a window into which word orders are possible within the verb phrase. With dative recipients, both RT and TR word orders are possible in topicalised VPs:
	...
	\begin{exe}
		...
		\begin{xlist}
			...
			\ex[*]{Where did you give the package to?\pagebreak}
		\end{xlist}
...
\begin{exe}
	...
	\begin{xlist}
		...
		\ex[*]{Mary placed the planting pots$_{k}$ under the tomato vines$_{i}$ [PRO$_{i}$ to grow over e$_{k}$]\pagebreak}
	\end{xlist}
...
\pagebreak
	...
	\begin{exe}
		\ex[ ]{John's kindly giving (*of) a book *(to) Mary.}
			\ex[?]{John's kindly giving *(to) Mary (*of) a book.}
	\end{exe}
