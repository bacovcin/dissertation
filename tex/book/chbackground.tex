...
\text{Mary} & \text{v} & & ApplP\ar@{-}[dl]\ar@{-}[dr]\\
 ...
 \begin{exe}
	 \ex Head Movement Targeting Condition: When head movement is triggered, the head triggering the movement targets the nearest head (or phrase) that asymmetrically c-commands the highest projection of the triggering head. The triggering head then adjoins to the target (or to the head of the target if the target is a phrase). \label{ex:headmovement}
 \end{exe}

 The condition in (\ref{ex:headmovement}) was created to satisfy two requirements. The first is to guarantee that a higher specifier would intervene in head movement out of a lower specifier. For example, the theme after VP-internal scrambling would be the target of P-incorporation from the recipient, since the D-head of the theme is the nearest head that asymmetrically c-commands the recipient (\ref{ex:doubspec-tree}). Without including the possibility of a phrase being the target of head movement, the theme D-head would not c-command the dative P head, since it would be embedded inside the theme DP.\footnote{As an aside, the theme is only a target, because it is in a higher specifier of the applicative phrase. If the theme were moved into a different functional projects (e.g., ScramP) attached above the applicative, then the Scram head, rather than the theme would be the target:
...
\text{Mary} & \text{v} & & ScramP\ar@{-}[dl]\ar@{-}[dr]\\
 ...
 & & \text{the book$_{i}$}\ar@{<-}@(dl,dl)[dddrrrr]& Scram & & ApplP\ar@{-}[dl]\ar@{-}[dr]\\
 & & & & PP\ar@{-}[d] & & \bar{Appl}\ar@{-}[dl]\ar@{-}[dr]\\
 & & & & \text{to John} & Appl & & VP\ar@{-}[dl]\ar@{-}[dr]\\
 & & & & & & DP_{i} & & \bar{V}\ar@{-}[d]\\
 & & & & & & & & V\ar@{-}[d] \\
 & & & & & & & & \text{give}}
 ...
 The second requirement is that the highest position of the triggering head is the phrase level, which insures that heads will not head move into their own specifiers. Without requiring that the target c-command the \textbf{highest projection} of the triggering head, the specifier would asymmetrically c-command the head of the phrase it is contained in and thus would be a valid target for head movement (\ref{ex:specifierBPS}). Instead, the specifier is a daughter of the highest projection of the triggering head and thus does not meet the requirements for c-command.
